...
class Catalogo_tipos_consulta:
    ...
    def obtener_por_id(self, id_tipo):
        ...
        """
        resultado = db_connection.ejecutar_consulta(query, (id_tipo,))
...
class Catalogo_aseguradoras:
    ...
    def actualizar_descuento(self, id_aseguradora, nuevo_descuento):
        """Actualiza el porcentaje de descuento de una aseguradora"""
        query = """
            UPDATE aseguradoras 
            SET porcentaje_descuento = %s 
            WHERE id_aseguradora = %s AND activo = TRUE;
        """
        filas_afectadas = db_connection.ejecutar_actualizacion(query, (nuevo_descuento, id_aseguradora))
        return filas_afectadas and filas_afectadas > 0

    def actualizar_multiples_descuentos(self, aseguradoras_actualizadas):
        """Actualiza múltiples descuentos de aseguradoras"""
        try:
            actualizaciones_exitosas = 0
            for aseguradora in aseguradoras_actualizadas:
                if isinstance(aseguradora, dict):
                    id_aseguradora = aseguradora.get('id_aseguradora') or aseguradora.get('id')
                    descuento = aseguradora.get('descuento_general') or aseguradora.get('descuento_porcentaje')
                    
                    if id_aseguradora and descuento is not None:
                        if self.actualizar_descuento(id_aseguradora, descuento):
                            actualizaciones_exitosas += 1
                            print(f"✅ Descuento actualizado para {id_aseguradora}: {descuento}%")
                        else:
                            print(f"❌ Error actualizando descuento para {id_aseguradora}")
                    else:
                        print(f"❌ Datos incompletos para aseguradora: {aseguradora}")
            
            print(f"📊 Total actualizaciones exitosas: {actualizaciones_exitosas}/{len(aseguradoras_actualizadas)}")
            return actualizaciones_exitosas > 0
        except Exception as e:
            print(f"❌ Error al actualizar múltiples descuentos: {e}")
            return False
